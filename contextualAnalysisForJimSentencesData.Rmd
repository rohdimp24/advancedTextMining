...
subcases<-dbGetQuery(mydb, "SELECT * FROM `smartsignal_jim_allfields` LIMIT 2000")
...
del=c("brgx vibes","brgy vibes","heat valve","top level","differential difference")
...
outputMatrix=matrix(nrow=1000,ncol=7)
for(i in 1:NROW(totalPotentialAssets))
{
  print(i)
  #i=1
  #input asset
  nounPhrase=totalPotentialAssets[86]
  ...
  if(!is.null(dim(res)))
  {
    ...
    outputMatrix[i,2]=paste0(names(mostFrequentVerbs[mostFrequentVerbs>0]),collapse = ",")
    outputMatrix[i,3]=paste0(mostFrequentVerbs[mostFrequentVerbs>0],collapse = ",")
    #docNum=rownames(res)
    #dd=head(docNum)
   ...
   frequentlyOccuringNouns=findMostCommonNounOccuringAfterNoun(nounPhrase,totalUnigramNouns,dtmMat,totalPotentialAssets,finalTxtMat)
    ...
    outputMatrix[i,7]=paste0(format(round(frequentlyOccuringNouns/C_W1,2),nsmall = 2 ),collapse = ",")
...
write.csv(outputMatrix,"output_jim10.csv")
